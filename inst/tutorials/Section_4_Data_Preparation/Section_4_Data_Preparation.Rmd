...
status_untidy <- readRDS("data/untidy2.rds")
```
...
Data can be represented in multiple ways.  Tidy data may not always be the optimal format for visual inspection, reporting or for data entry for example, but it is ideal for analysis. All packages in the **`tidyverse`**, such as **`dplyr`** and **`ggplot2`** for example, are designed to work with tidy data.  
...
Below are examples of the same data, with 4 variables, **country**, **year**, **population**, and number of documented **cases** of TB (tuberculosis), represented in 3 different ways.  The last table, **Table 3**, is in tidy data format, and it is the easiest to use for analysis. 
...
You can check with the with the tidy data criteria that **Table 3** is in tidy format, this is also shown graphically below.
...
The table below represents some student exam results:

```{r, echo = FALSE,out.width = '100%'}
#, fig.width=474, fig.height=111
knitr::include_graphics("images/untidyex4.png")
```
  

First try and identify the different variables represented in the table, then answer the following questions to decide if each of the tidy data statements are true or not for the table:
...
quiz(caption = "",
  question("Each variable has its own column.",
    ...
    allow_retry = TRUE,
    correct = "Correct. This table has 4 variables: student ID, subject, year, and grade; these variables are not in their own columns.", 
  ...
  question("Each observation has its own row.",
    ...
    answer("FALSE", correct = TRUE),
    allow_retry = TRUE,
    correct = "Correct. The student results for each year and subject are independent observations, and should be in their own rows.",
    incorrect = "Incorrect, try again."
  ),
  question("Each value has its own cell.",
    ...
    answer("FALSE"),
    allow_retry = TRUE,
    correct = "Correct. Each grade value is contained within its own cell.",
    incorrect = "Incorrect, try again."
  ),
  question("Are there any units of measurement included?",
  ...
  question("Is there any extraneous information included?",
    answer("YES", correct = TRUE),
    answer("NO"),
    allow_retry = TRUE,
    correct = "Correct. This table includes an asterisk and a footnote, this is extraneous information.  This information should either be captured in a different way (such as in the associated metadata).",
    incorrect = "Incorrect, try again."
  )
  )
```
...
This table shows the release status for different batches of a medicine produced in a pharmaceutical manufacturing facility:  
...
quiz(caption = "",
  question("Each variable has its own column.",
    ...
    answer("FALSE", correct = TRUE),
    allow_retry = TRUE,
    correct = "Correct. This table has 4 variables: year, campaign, batch and release status; these variables are not in their own columns.", 
    incorrect = "Incorrect, try again."
  ),
  question("Each observation has its own row.",
    answer("TRUE", correct = TRUE),
    answer("FALSE"),
    allow_retry = TRUE,
    correct = "Correct. The release status, year, campaign, batch are each in their own rows ",
    incorrect = "Incorrect, try again."
  ),
  question("Each value has its own cell.",
    answer("TRUE"),
...
- `remove` removes the original column you are separating, it defaults to TRUE so we don't usually include it, however if you want to retain the original column in the dataframe you can include it and set it to FALSE.  
...
The following illustrates a straightforward example where `separate()` can be used.  Here we have a dataframe called **results** with a list of surnames and first names in a single column called **names**:
...
We can use `separate()` on the **names** variable to split it into 2 new columns called **surname** and **first_name**. We also specify using the `sep=` argument how we want to split the data, in this case splitting at the comma. The `separate()` function will automatically remove the original **names** column unless the argument `remove = FALSE` is present.
...
Note this expression generates a new dataframe, but it doesn't save it.  We can either replace the original dataframe (by saving to an object called **results**) or save it to a new dataframe object, for example **results_tidy**, as shown here: 
...
Here we have an example dataframe from the quiz at the start of the tidy data tutorial, we have called it **status_untidy**. Let's see how we can use the function `separate()` as a step in transforming it from *untidy* data into *tidy* data. 
...
The first column contains three different variables of **year**, **campaign** and **batch**, and the `separate()` function can be used to separate this into 3 new columns.  The **Year-campaign-batch** column is still very useful in this context though as the company needs it as unique identifier in its data, so we need to also retain this column.

Separate the first column into new columns called **year**, **campaign** and **batch**, and also retain the first column as a unique ID column:

```{r untidy-S1, exercise=TRUE, exercise.lines = 7}
...
```{r untidy-S1-hint}
# col to separate: "Year-campaign-batch"
# new variables: c("year", "campaign", "batch")
# separator: "_"
# set the remove argument to FALSE to retain the original column     
```


```{r untidy-S1-solution}
# Solution: 
#Complete this expression: 
status_untidy %>% 
...
We can now see that we have three new variable columns made from the first column, and that we have retained the first column as a unique ID variable. In the exercise at the end of the next section we will see how to complete the transformation of this untidy data into tidy data format.
...
We can illustrate how to use `pivot_longer()` on the same **results** dataset we used to demonstrate the `separate()` function.  This shows a list student names and the results they received in semesters 1 & 2:
...
This results in tidy data, that satisfies the tidy data rules:
...
Here we have the example dataframe, **status_untidy**,  from the quiz at the start of the tidy data tutorial, and this is also the dataframe used in the `separate()` exercise. Let's step through it to apply the functions `pivot_longer()` and `separate()` and to transform it from untidy data into tidy data. 
...
Use `pivot_longer()` to lengthen this dataframe by combining the column names **Approved**, **Reject**, **In-Quarantine** into a single column called **status**, and the values of those columns into a column called **values**:



```{r untidy-1, exercise=TRUE, exercise.lines = 7}
#Complete this expression: 
status_untidy %>% pivot_longer(
...
Once we lengthen this data we see that we only need to keep the rows that have "X" in the values column as those that don't hold no release status information.  
...
We have successfully captured the **status** for each **Year-campaign-batch** however the column called **values** now shows no useful information (all values are now "X"), so we need to remove it.
...
This is already looking a lot neater, we have a column for the batch identifier and the status of that batch.  It is easier to interpret and to analyse.  

In the `separate()` section we did an exercise to separate the **Year-campaign-batch** column into three new variables of **year**, **campaign** and **batch**.  We also retained the **Year-campaign-batch** column as the company needs it as unique identifier in its data. 

We can now combine this `separate()` step with steps 1 to 3 to further tidy the data. 

#### Step 4

This code includes both `pivot_wider()` and `separate()` steps. Run the code block and observe the output:


```{r untidy-4, exercise=TRUE, exercise.lines = 13}
...
status_untidy %>% 
  ...
  values == "X"  
    ...
    -values
  ) %>% # code included from separate() exercise:
  separate("Year-campaign-batch", 
           into = c("year", "campaign", "batch"), 
           sep = "-",
           remove = FALSE)
...
Finally, we can add an optional step to complete the tidying and cleaning of the data.  For example we could update the column types, and rename the first column, to give a final tided dataframe.  

Note is is vital to save your dataframe, here we have saved it to a new dataframe object called **status_tidy**. 

The final code to tidy and clean the data (steps 1 to 5), and the resulting dataframe is shown below:

```{r, echo = T, eval = T}
status_tidy <- status_untidy %>% # save dataframe to a new object
  pivot_longer(cols = c("Approved", "Reject", "In-Quarantine"),
  names_to = "status",
  values_to = "values"
  ) %>% filter(
...
The third useful function from `tidyr` is `pivot_wider()`.  This makes datasets wider, although used less commonly in tidying data, it is useful when one observation is spread across multiple rows.  It is also very useful for when data is needed in an 'untidy' wide format for other purposes, often tidy data is not ideal for presenting or reporting for example, or it is needed in a particular format to suit a specific modelling technique.  

It is the inverse operation to `pivot_longer()`.
...
In the following example there are minimum and maximum weight measurements, recorded from a sample of fish collected from two different rivers for two different fish species. This form of data could be considered tidy, and is ideal for use with `ggplot()` for example.  

There are instance where *widening* the data may provide tidy data, for example if we consider these fish measurements as a single observation representing the weight range of the sample of fish being taken from the same sample at the same time. 

More commonly we want the data in a format suitable for further modelling, for example we may want the minimum and maximum weights as separate variables. *Widening* data can be very useful also when transforming tidy data into tables suitable for presentations, reporting, or for easier data entry for example. 
...
To *widen* this data we can use `pivot_wider()`.  The **measure** is the *key* column, it contains the variables names, and these will be assigned as the names of two new columns (**max** and **min**).  The *values* are the different fish weights.
...
This data is now in the desired format.

To further tidy and clean the data we can also remove the units "kg", and convert the weights variable from character to numeric, and change species and river to factor variables:
...
Dates and times can be also represented in data in different formats such as strings or numbers, and in many different ways. For example, the following all represent the same date: 
...
Lubridate is not part of the core tidyverse as it is only required when working with dates/ times. Therefore to use it we need to install the lubridate package and load the library.
...
We can use **lubridate** functions to convert the date strings or numbers into the standardised `date` format.  The lubridate functions are constructed using letters that represent the date elements in the order that you observe them appearing as in the data.  The date elements are indicated using the following abbreviations: 
...
All of the dates in the following examples are character strings that represent the same date, the **13th March 2024**.

To convert the strings into the standarised `date` format we can use lubridate functions made up from **d**, **m**, and **y** to specify which order the date elements appear in each string. 

The date in this example is in *day*, *month*, *year* order, so the function `dmy()` is used: 
...
Note the output is now in the standardised `date` format.

The functions can handle most delimiters as well as extraneous information such as the "th" in this next example.  This date is in the order *year*, *day*, *month* so the function `ymd()` can be used: 
...
:::: {.notesbox }
**N.B.** If the data only contains month and year, with no day information, when converted by lubridate the date will default to the first of the month.
::::
...
Convert the following three date strings into the standardised `date` format using lubridate functions.  These date strings all represent the same date, the **15th October 2023**:
...
Dates represented by numbers can be converted to `date` format using lubridate in the same way as we did for strings. 

In this example the date is in the order *year*, *month*, *day*, so the lubridate function `ymd()` can be used to convert it into `date` format: 
...
This number represents the same date in a different order:
```{r, eval=TRUE, echo=TRUE}
date6 = 031324
mdy(date6)
```
...
Convert the following dates, that are represented by numbers, into the standardised `date` format using lubridate functions.  These numbers all represent the same date, the **15th October 2023**:

```{r, datestrings-2A, exercise=TRUE, exercise.lines = 6}
# The numeric date to be converted:
date_e = 231015
...
To include times with the dates we can add the abbreviations **h**, **m**, **s** to the function.  We can then use lubridate functions to convert dates into `date-time` format (this will be either `dttm` or `POSIXct` class). 
...
Similarly to dates, we can convert either character strings or numbers to `date-time` format. 
...
Using lubridate functions convert the following three date-times into the standardised `date-time` format.  They all represent the same date-times (if the minute and second data is available), the **05th April 2021 @ 21hours 40mins 27sec**
...
In lubridate, date-times are automatically stored with the UTC timezone.


We can use the argument `tz =` within the lubridate functions to specify whatever timezone we choose.  It can take both country names and letter abbreviations, and lubridate will always use an abbreviation to store the dates.

For example we can specify a specific timezone using an abbreviation:

```{r, eval=TRUE, echo=TRUE}
mdy_hms("June 5th 2021 15:14:30", tz = "GMT")
```

Or we can specify a specific timezone using a country string, lubridate will store this in its abbreviated form:
...
The function `OlsonNames()` can be used to look up the list of ~ 600 country names and abbreviations that lubridate accepts.
...
:::: {.extrabox }
Although dates can be accessed in any order, times can only be used in the order of *h-m-s*.  The package  [`hms`](https://hms.tidyverse.org/reference/hms-package.html) is an alternative package within the tidyverse for dealing with only times. 
...
Once the date or date-times are converted into either `date` or `date-time` format we can then use a range of **get** and **set** functions to access and change different elements of the dates and times. 

Examples of **get** and **set** include the following self-explanatory functions; these can be used to **get** or retrieve elements of dates and date-times, or to **set** or change elements. 
...
A full list of potential lubridate **get/set** functions can be found [**here**](https://lubridate.tidyverse.org/reference/index.html).


### Get 


**Get** functions retrieve specified elements from dates and date-times so long as they're in the standardised lubridate `date` and `date-time` formats. 

We will use a formatted `date-time` called **mydate** here to demonstrate some of the **get** functions. 

For example, to get the day of the month from a `date-time` we can use the function `day()`:

```{r, echo = TRUE, EVAL = TRUE}
# first convert the date-time
...
Within some functions we can also specify, where applicable, if we want the element outputted as a string (using the argument `label = TRUE`) and if we want it abbreviated (`abbr = TRUE`).
...
In addition to the straightforward functions listed above, we can also use a range of functions to **get** different information from dates and date-times such as:
...
For example, to **get** the day of the week as an abbreviated string we can use `wday()`. 
```{r, echo = TRUE, EVAL = TRUE}
...
# The function month() is needed here.
...
The same functions as for **get** can be used to **set**, to change,  different date and time elements. 

The full list of potential lubridate **get/set** functions can again be found [**here**](https://lubridate.tidyverse.org/reference/index.html).

Again we will generate a `date-time` called **mydate** to demonstrate the functions.

For example we could change the year of a date by assigning a new year value to the `date-time` object **mydate** using `year()`: 

```{r, echo = TRUE, EVAL = TRUE}
# first format the date-time
mydate <- mdy_hms("June 5th 2021 16:14:30", tz = "GMT")
...
Notice the year in **mydate** has changed from 2021 to 1999.
...
mydate
```
`tz()` simply changes the timezone in the `date-time` (here from GMT to EST) but doesn't *convert* the original date-time to the new timezone.  To do this, the function `with_tz()` can be used:
```{r, echo = TRUE, EVAL = TRUE}
...
We can see here that the time and date of the **mydate** `date-time` have been changed to reflect the new timezone. 
...
Use lubridate functions to **set** the specified elements of this date-time **2024-01-29 19:31:15 UTC**:
...
# The function day() is needed here.
...
# The function year() is needed here.
# Don't forget to call the date to see the answer.
...
This sample data set, **invoicing**, can be used to apply the methods learnt.  The dates in this dataframe are in character (string) format.  In order for R to recognise and work them as dates they need to be converted into a format that R understands.  We will use lubridate to convert them to `date` format (this may be either `dttm` or `POSIXct` classes - these are equivalent). 
...
The code block below shows how to convert a single column to `date` format. Here we will use lubridate to convert the column **Invoice**, which is a character `chr` class, into a new column called **Invoice_date** which is in `date` class. 
...
Using `dplyr` verbs we could convert all the columns we want to select to `date` format. The code block below specifies that columns 2 to 4 are to be converted to `date` format using the lubridate function `dmy()`: 
...
Notice that all date columns are now in `date` format.

Remember, if we want to keep the changes we've made to a dataframe we need to either save it to a new dataframe or replace the existing dataframe. For example, here we save the new dataframe to a new dataframe object called **invoicing_2**. 

```{r, eval = T, echo = T}

invoicing_2 <- invoicing %>% mutate(across(2:4, ~dmy(.)))
...
The `difftime()` function is part of base R and is useful for calculating periods between date/date-times when they are in `date` or `date-time` class.

These are the order and delivery dates for an online order converted to `date` class using lubridate: 
```{r, eval = T, echo = T}
...
We can also just use simple addition or subtraction on the dates and date-times, this can be especially useful when working with dataframes/ tibbles.
...
We will use this example `date-time`, and call it **event_date**, to demonstrate some examples of adding or subtracting date/time elements:
```{r, eval = T, echo = T}
...
We can apply these arithmetic functions to our sample data set **invoicing**.  We can calculate the time periods between **Invoice** and **Due** dates for example.  Using the dplyr function `mutate` we can save the result to a new variable called **invoice_duration**:
...
Observe we now have a new column called **invoice_duration**, and this is returned as a new class called `drtn`, this stands for *duration*. 
...
Use the sample data set **invoicing_ex** to calculate the time duration between the **Due** date and **Payment** dates. Save this new variable to a column called **delay**. (Note the date columns are already in `date` format).
...
Again remember, if we want to keep the changes we've made to a dataframe we need to either save it to a new dataframe or replace the existing dataframe. 
...
This can be in addition to retaining the `date` or `date-time` variable in a single column so that lubridate functions such as the ones explored above can be used.  
...
To make tidy date data we generate new columns using the lubridate **get** functions `year()`, `month()`, `day()`.  
...
```
The **invoice** column can be formatted into tidy date format by creating new **year**, **month** and **day** columns  with the dplyr function `mutate()` and populating them using **get** functions:
...
Observe the columns.  We can then save this dataframe to a new dataframe, or replace the existing dataframe.

### Tidy data to date/date-time format

Sometimes date data is stored only in tidy format such as the table below. Note here, for example, they are stored as numbers.  To convert this back to `date` or `date-time` format we can use the lubridate functions `make_date()` or `make_datetime()`.

This **tidy_dates** dataframe is in tidy format: 

```{r, eval = T, echo = T}
tidy_dates 
```

As there is no time element, we can use `make_date()` to construct a new column using `mutate` called **Date** in `date` format from the **year**, **month** and **day** columns:
...
Observe that there is now a new column called **Date**, in `date` format. 

:::: {.exercise-box }
### Exercise

Using the sample dataframe below, called **tidy_datetimes**, create a new column called **datetime** that contains *both* date and time elements in `date-time` format.  
...
We can use the `as_date()` or `as_datetime()` function with an `origin = ` argument to convert these numbers to `date` or `date-time` classes.

An example to convert an Excel date integer:

```{r, eval = T, echo = T}
as_date(45046, origin = "1899-12-30")
```
Or to convert Unix date integer for example:

```{r, eval = T, echo = T}
as_date(19477, origin = "1970-01-01")
```
Or to convert a UNIX date-time integer:
...
:::: {.readingbox }
A great place to read more about working with dates is Chapter 17 in [R for Data Science (2e)](https://r4ds.hadley.nz/).  In addition to the [lubridate](https://lubridate.tidyverse.org/index.html) webpage the [lubridate cheat sheet](https://evoldyn.gitlab.io/evomics-2018/ref-sheets/R_lubridate.pdf) is a handy resource.
::::

:::: {.readingbox }
The best place to read more about working with tidy data is Chapter 5 in [R for Data Science (2e)](https://r4ds.hadley.nz/data-tidy).
::::
